...
The concordance of names across v2, v3, §28, THE LOCK, v4, and the code: the collisions to settle before a name freezes (`bridge` and `propose` vs `patch`, settled 2026-09-07; `tenant`, half settled; `receipt`, `bless`), the actor grammar by version, every event kind's North Star name beside the code's, every verb list side by side, and the terms the code has that no North Star names.
...
The house's decision log, opened 2026-09-06 at the charter's ask: one entry per architectural decision with the reason, the alternatives rejected and why, and what would reopen it. Seeded with the six decisions of Phase 6, including the fold's measured quadratic cost held with its trigger, and extended with D-007 through D-011 for the synthetic proof (the firewall, the log-seam stamp, the discrimination baseline, and the fold's breach refined to 1,150–2,800 events). Decisions made before this file lived inline in the specs and stay there.
...
| `CONSTELLATION_HORIZON.md` | Exists | Technical envisioning paired with `CONSTELLATION.md`. The endpoint described, decomposed into five rendering layers (atmospheric / labeling / structural / strata / audio), the stack chosen and refused (WebGL via `ogl`, SVG for bones, Web Audio synthesized; refusing `react-force-graph`, Three.js, pure-Canvas-for-structure), the data contracts each layer needs, build-vs-runtime allocation, performance and accessibility invariants, and a ten-phase migration path from the current state to the finished surface. Working backwards from the endpoint. |
| `CONSTELLATION_DESIGN.md` | Exists | Designer's working document for the constellation surface. Enumerates the surfaces /sky can be in (S0–S19), the components that compose them (cursor glyph, star, thread, atmosphere pool, horizon strip, search field, facet chip, time scrubber, radial echo, polestar panel, work overlay, etc.), the canonical flows (first arrival, wander-and-open, filter-and-traverse, search-and-arrive, pin-and-trace, time-scrub, thread-traversal, idle-to-contemplative, reduced-motion), and design variants for each held question — so a designer can sit with the document and start producing hi-fi mockups, storyboards, and proposals. The bridge between `CONSTELLATION.md`'s held experience and the artifacts a designer produces. Carries semantic-anchoring layers (aesthetic references, semiotic loads, epistemic posture, metaphorical grammar, stylistic register map, compositional eye-movement) so downstream agents can make design decisions that respect meaning, not just visual style. Cross-trunk: outside (interaction, design language, voice) and inside (the domain it visualizes). |
| `CONSTELLATION_IMPLEMENTATION_AUDIT.md` | Exists | Diagnostic survey of the gap between `CONSTELLATION_DESIGN.md`'s commitments and the codebase as it stands. Status markers (present, partial, absent, drift, held) for each surface, component, behavior, foundation principle, aesthetic/semiotic/epistemic/stylistic commitment, and architectural prerequisite. Names the spec/code drift across the three constellation documents and the test-coverage drift. Distinguishes itself explicitly from a plan — the audit names the gap, a plan would choose how to close it. Read alongside `CONSTELLATION_DESIGN.md`; written so a downstream plan can reference its rows. Cross-trunk: outside (the design read it audits against) and the house (the architectural readiness it surveys). |
| `CONSTELLATION_IMPLEMENTATION_PLAN.md` | Exists | The chosen sequence through the gap named by the audit. Eighteen numbered phases (P0–P13, with P3.5, P3.6, P3.7, P3.8, P10.5, P12.5, P12.6, P12.7 inserted) covering foundation refinement, world-layer polish, arrival/departure gestures, the chrome substrate, the chrome surfaces in pulled order (with a chrome surface template artifact shipping alongside P5 — the first chrome surface — so P6/P7/P8/P9/P10/P10.5 inherit a state-machine, focus, dismissal, and scene-broadcast contract rather than re-derive one), thread traversal, the living-document behaviors, the daystar's ascent, the /sky-/facet reconciliation, the high-node-count strategy, and full spec reconciliation. Below the numbered phases, a Held register appendix collects held-but-visible commitments (offline-state behavior; named modes not yet ridden) and beyond-the-plan vocabulary — documentation-only, no implementation expected. Each phase carries pull condition, scope, exit condition, architectural decisions made within the phase, parallel tracks, what-stays-held, specs-to-reconcile, and tests-to-land. Performance budget leads the cross-cutting concerns (215 KB ceiling, long-task delta gate, Lighthouse floor, test-suite runtime budget; the bundle-ceiling prediction surfaces a likely route-split for /sky around the P5–P6 era), alongside accessibility, spec reconciliation, voice, semiotic load, density, eye-movement, drift-mode resistance, aesthetic touchstone, and epistemic posture. Honors the *spanda* practice: phases are sequence under the assumption that pulls arrive, not a calendar. Reads as snapshot; updates as phases ship. Cross-trunk: outside (the design read it sequences) and the house (the architectural commitments it preserves). |
