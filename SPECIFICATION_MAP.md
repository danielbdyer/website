...
| `CONSTELLATION_IMPLEMENTATION_PLAN.md` | Exists | The chosen sequence through the gap named by the audit. Twenty named phases (P0 through P15, with P3.5, P3.6, P3.7, P3.8, P10.5, P12.5, P12.6, P12.7 inserted) covering foundation refinement, world-layer polish, arrival/departure gestures, the chrome substrate, the chrome surfaces in pulled order, thread traversal, the living-document behaviors, the daystar's ascent, the /sky-/facet reconciliation, the high-node-count strategy, full spec reconciliation, the held register, and beyond-the-plan. Each phase carries pull condition, scope, exit condition, architectural decisions made within the phase, parallel tracks, what-stays-held, specs-to-reconcile, and tests-to-land. Cross-cutting concerns (accessibility, performance, spec reconciliation, voice, semiotic load, density, eye-movement, drift-mode resistance, aesthetic touchstone, epistemic posture) span every phase. Honors the *spanda* practice: phases are sequence under the assumption that pulls arrive, not a calendar. Reads as snapshot; updates as phases ship. Cross-trunk: outside (the design read it sequences) and the house (the architectural commitments it preserves). |
