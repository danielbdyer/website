...
**The log.** One append-only log per tenant. Every event carries its step in that log, the time it was recorded, and the space it belongs to. The working state — spaces, verbs, receipts, reflections, crossings, bridges, references — is a fold over the log, and folding the same log twice yields the same state. Nothing else is ever written. In this repository the log is markdown and JSON in git, per `CATHEDRALS.md` §"Git Is the Vessel"; the sidecar's SQLite is an index of it, rebuilt from it.

**The tenants.** Two spaces from the first day: an operator space and an agent space, each with a sovereign, and each sovereign over itself. Inside its own space a tenant writes without asking, and answers at once: the agent is a tenant, sovereign over its own space, and a relation it asserts there is blessed in the same call (D-018). A node moves between spaces only by a *crossing*, which materializes as pending in the target space and waits for the target's sovereign. A *bridge* is a relation with evidence — subject, predicate from the slice's closed set, object, and the span or observation that shows it — proposed by a session into the space it names, waiting there for that space's sovereign, and blessed is an edge in that space's slice; the operator's own wiki links are the same edge reached by writing (D-016, D-019). A weak reference is the other way across: a citation written onto the relating node, in its own space, pointing at a node across the wall by text — a historical textual citation, resolvable by the fabric and never an edge. The vault's rule for a reference, that it is a receipt and not the content, holds here at the scale of two graphs.
...
4. A crossing offers the compiled claims to the operator space. Pending, with the gap held.
...
| Candidates and bridges | ActiveGraph's core pack; the first slice | `memory_candidate`, then `evaluation`, is `reflection.recorded`, then `crossing.proposed`, then a decision. Held until the runtime enters as a tool over the log. |
...
| Approval and blessing | ActiveGraph's `propose`, `pending_approvals`, `approve(approved_by)`; the fabric's `propose`, `pending`, `bless` | `approval.proposed` to `granted` or `denied` is `crossing.proposed` to `resolved` with `decision` and `by`. Nothing translates. |
...
| Retrievals and citations | the corpus charter's R(t): a stored item surfaced outside its creation context that influenced the next action; the fabric's `retrieval.surfaced` candidates and a reflection's `cites` | The definition names two events and a join. The fabric already had the second (a session cites what it read) and adds the first; the join is a fold, and the number is in `describe` beside graduation. Nothing was translated: the charter's definition is the fold's definition. |
| Actors and because | the charter's `author \| agent:<id> \| import:<source>` with `because` required of agent events at the schema; the fabric's actor grammar and the `because` refinement on the event union | One regular expression and one refinement. An event that violates either is not parsed, so the file log cannot hold one. |
...
- **INV-FAB-003 — a crossing crosses a wall and is closed once, by the target's sovereign.** `from` and `to` differ; the first resolution wins; a resolution by anyone but the target's sovereign is reported. (Named a bridge proposal until 2026-09-07; D-016.)
...
- **INV-FAB-006 — a tenant sees its own space whole and the other space through blessing.** Inside its own space a session retrieves everything it recorded. Across the wall it retrieves only what a crossing carried over and the sovereign blessed. Private evidence never leaks through retrieval.
...
- **INV-FAB-010 — every retrieval is an event.** A verb that surfaces memory to a session (`slice`, `recall`, and the start hook's `orient`) appends `retrieval.surfaced` with the context it was asked in and every candidate in rank order, with its score and why it was surfaced; a receipt for a retrieval verb with no retrieval event beside it is reported. This is the raw material of R(t), and it is written before anything is built to raise it.
- **INV-FAB-011 — provenance is complete at the schema.** Every event names its actor in a closed grammar — `runtime`, `author:<space>`, `agent:<session>`, `import:<source>` — and an agent event carries a `because`, one sentence, or the schema refuses it; the invariant check reports one that was hand-edited in. A receipt's `because` is the sentence the calling session gave, read off the verb's input, so every verb asks for one.
- **INV-FAB-012 — a bridge relates two distinct nodes with evidence, is closed once by the sovereign of the space it lands in, and blessed is an edge in that space's slice.** `subject` and `object` differ, or the verb refuses the call; the first resolution wins; a resolution by anyone but the space's sovereign is reported; a session's own space resolves at proposal, by the session's space as author. The blessed bridges of a space are its `declared` edges, drawn wherever both ends are drawn.
...
- **Scope:** The agent space opened; the crossing on `reflect`; `hg bless` resolving it; the retrieval filter live. Weak references written on reflections that cite the vault.
...
- **Scope:** Change requests as proposals against prompt, skill, verb, and policy nodes; a held-out evaluation before promotion; the graduation boolean stated in advance.
- **Exit:** One change to a skill lands through the gate, with its receipt and its evaluation in the log.
- **Shipped, 2026-09-06:** the mechanism, waiting on the first real patch for its exit. A patch is a node's whole new text against the base it was read at, with *because* and a *hypothesis* the next session can check. `patch` (named `propose` until 2026-09-07, D-017) reads the base through the canon port, appends `patch.proposed` into the operator's space, and appends the fabric's own `patch.evaluated` — base unchanged, body differs, frontmatter parses, `markdownlint-cli2` and `cspell` as a ratchet on a base and a patched scratch copy (a patch may carry no finding the base does not already carry, so a node the repository never lints is held to itself) — before Danny sees it. `pnpm fabric pending` shows the patch as a diff with its checks; `pnpm fabric bless patch/<id>` applies it to the canon only if the base still matches, then records `patch.resolved` with `applied`; `reject` keeps it as data. `orient` shows the next session every applied patch nobody has measured, with its hypothesis, and that session reports through `reflect.outcomes` as `confirmed` or `contradicted`, which lands as `patch.outcome`. Graduation is `{ floor: 0.5, window: 5 }`, computed in `describe` and printed in `fabric/README.md`; it gates nothing. The evaluation is the fabric's checks rather than a held-out fork of the runtime: a change to a skill has no test set, so what can be verified without a reasoner is verified, and the hypothesis carries the rest to the next session. Today the canon holds the skills under the blessed skills source, found by frontmatter name; prompt, verb, and policy nodes are targets the schema admits and the canon does not yet hold.

---

### Phase 6 — The instrument

- **Scope:** Measure whether the corpus compounds before building anything meant to make it. The corpus charter's invariant (`CORPUS.md` §0): a corpus compounds iff outputs become inputs, R(t) being the retrievals where a stored item was surfaced outside its creation context and influenced the next action. In the fabric that is two events and a join: `retrieval.surfaced`, appended by every verb that surfaces memory with its candidates in rank order, and the citations a later reflection or patch by the same session makes, counted only when another session made the node. Actors in a closed grammar and a `because` on every agent event, refused at the schema (INV-FAB-011).
- **Exit:** R(t) is a number in `describe`, in `fabric/README.md`, and in the first line the start hook prints, folded from the log alone; the four properties the charter names — fold determinism, portable-format identity, blessed-versus-proposed, provenance completeness — are property tests over generated logs, green.
- **Shipped, 2026-09-06:** the exit, on the day the charter arrived. `packages/fabric/src/compounding.ts` folds the log into retrievals, used, rate, hit@k, mean reciprocal rank, missed (nodes a session used that nothing surfaced to it), a per-session series so the slope is visible, and proposal acceptance (blessed over decided, for the charter's §4 band). `describe` carries it; the README prints it; `orient` prints one line of it and tells the session that citing what it read is how the number is measured. Every verb input gained `because`, the one field every verb asks for; a receipt carries it; the refusal carries its reason. `fast-check` entered as a dev dependency for the property tests (`packages/fabric/src/compounding.test.ts`; sixty generated logs a property; removal cost: three test blocks). What is not yet measured, in numbers: R(t) itself is zero, because no verb is blessed and no session has retrieved anything through the fabric; the instrument is built and the first reading waits on the first blessing.

### Phase 7 — Synthetic proof

- **Pull:** The instrument is built and R(t) is 0 of 0; before a real session accumulates, prove the loop closes, that the metric discriminates good retrieval from bad, and how the fold scales — so the first real blessing is a small, understood act and not a leap.
- **Scope:** A generated corpus with planted ground truth, driven through the real `slice` and `reflect` verbs; a retriever whose quality is a dial; the metric swept against that dial; the fold characterized across event counts. All quarantined so the real R(t) stays 0 until a real blessing and a real citation.
- **Exit:** The metric discriminates — hit@k and MRR climb monotonically with planted retrieval quality — with a committed baseline; the firewall holds by construction and by test; the fold's breach point is a number with a decision beside it.
- **Shipped, 2026-09-06:** the exit. `packages/fabric/src/sim/` holds the generative model (planted targets, heavy-tailed hubs, an honest miss fraction, a seeded hash), the θ-parameterized synthetic resonance, the harness that drives the real shell, and the `import:synthetic` firewall. The discrimination sweep shows hit@3 climbing from 0.057 at pure noise to 0.850 at real signal (`fabric/sim/baseline.json`, regenerated by `pnpm fabric sim-baseline`); the fast form asserts the climb in CI. The firewall is two guarantees — every synthetic event is `import:synthetic`, and synthetic logs live only in temporary directories, never `fabric/spaces/` — each checked in `quarantine.test.ts`, which also folds the committed real log to a synthetic-free R(t) of 0 of 0. The fold's 100 ms breach is measured at 1,150 events (reflection-heavy) to 2,800 (realistic mix), held with a trigger at 800 events and a fix recommendation (D-011). What synthetic proof cannot do — show real usefulness, or validate that a citation means genuine use — is stated first, in `CORPUS.md` Part three §0. The handoff, the exact sequence that moves real R(t) off zero, is `CORPUS.md` Part three §5.

### Phase 8 — The names settled, and the bridge (shipped 2026-09-07)

- **Pull:** Danny's word on the four names held since v4 arrived: "I prefer relation-with-evidence, patch, agent as tenant, and bring back markdown-bridge if you feel it'll be useful. Let's prove it out as a service."
- **Scope:** `bridge` as v4 §2 means it; the carry-across renamed `crossing`; the verb that proposes a patch named `patch`; the agent's own space answering at once; the markdown bridge recognized where it already is. Proved through the real shell rather than described.
- **Exit:** A bridge refused before its verb is blessed, blessed at once in the session's own space and drawn as an edge there, waiting as a ghost in the operator's space and drawn as an edge once he blesses it, and counted as a use of both its ends; all in `shell.test.ts`, `fabric.test.ts`, and `compounding.test.ts`; the real log folding under the new vocabulary with no issue.
- **Shipped, 2026-09-07:** the exit. `bridgeSchema` and `bridge.proposed` / `bridge.resolved`; the `bridge` verb (consequence `propose`; INV-FAB-012) proposed into Danny's space, unblessed, beside `patch`; `crossingSchema` and `crossing.proposed` / `crossing.resolved` with the `Consent` port over them; blessed bridges as `declared` edges in the memory slice and in the composite graph source, so a bridge from a reflection to a vault claim is drawn once both are in the operator's slice; `verb.withdrawn` as the runtime taking back an unblessed proposal, which `init` now does for a proposal whose signature moved in the build; `pnpm fabric bless bridge/<id>` and `pending` showing bridges. The one rewrite the log has had — three `bridge.proposed` lines renamed `crossing.proposed`, payloads unchanged — is recorded with its digests in D-016, and `reflect`, whose output field moved from `bridge` to `crossing`, was retired, re-proposed, and re-blessed on Danny's behalf under his standing authorization. The markdown bridge is `node/sources.ts` as it stands: an author's wiki link is a `references` edge with author origin, the same edge a blessed bridge is (D-019). Two commands are Danny's: `pnpm fabric bless patch` and `pnpm fabric bless bridge`.

The plan for what comes after Phase 7 is `NORTH_STAR.md` (v4, 2026-09-06, kept verbatim at the root): its §23 is the build order, gated by the log, and its reconciliation against this file and the house is `CORPUS.md` Part four. The lineage it narrows is persisted beside it — `NORTH_STAR_v3.md`, `NORTH_STAR_v3.2.md`, `THE_LOCK.md` — and every unit of all three is a row in `NORTH_STAR_BACKLOG.md`, with `INVARIANTS.md` (what holds each invariant), `VOCABULARY.md` (where the names collide), and `NEXT_STEP.md` (step 2) beside it (D-015). Step 1 of that order is two-thirds done — `reflect` is blessed and the first real reflection is in the log — and its third clause, the first citation, is the next session's first act.
...
- **The use signal beyond citation.** A retrieval is used when the session cites or patches a candidate. The charter also counts opened, quoted, linked, and explicitly dismissed. The fabric has no "opened" (a slice is the opening) and no dismissal; a `dismiss` verb, or a `used`/`dismissed` field on `reflect.cites`, would sharpen the measure. Trigger: the first session whose reflection wants to say "I read it and it did not help." Scheduled: `NORTH_STAR.md` §23 step 2, as `retrieval.used` / `retrieval.missed` (D-014).
- **Aliases.** The charter's §3: a node may carry several names, including the vernacular the author uses, and lexical retrieval indexes them. The vault's plain-language lexicon is the same idea for the vault's coinages. In the fabric an alias is a proposal: an `alias.proposed` event naming the node, the name, and the sentence in which the author used it. Trigger: the first `recall` that misses because the author's word for a thing is not the node's. `NORTH_STAR.md` Appendix C keeps the same trigger.
- **Graph proximity in the fusion.** `slice` ranks by resonance then mention; the sky's presence ranks by strokes, shared facets, concordance, and distance; the charter asks for the two fused with a stated weighting. The weighting is unstated because there is no real retrieval log to tune it against yet — though the synthetic discrimination harness (Phase 7) can now tell one weighting from another before real retrieval exists, so a candidate fusion can be proposed against the baseline rather than guessed. Trigger: fifty real retrievals in the log, or a fusion change proposed and checked against `fabric/sim/baseline.json`.
- **Capture as a measured path.** The charter's §2 asks for a p99 of 16 ms from first keystroke to persisted event, with zero required structure and the raw input kept verbatim. The fabric has no capture surface: the author writes markdown into git, and the agent's capture is `reflect`, which is structured by design. Reflection is not capture. A capture verb — `note`, taking one string and nothing else — is the smallest real version, and its latency is the file log's append. Trigger: Danny's word that he wants to capture through the fabric rather than through his editor. That word arrived as `NORTH_STAR.md` §7, so D-005 reopens; scheduled at §23 step 3, because `note`'s 16 ms budget needs the daemon (D-014).
- **The canon beyond skills.** Prompt, verb, and policy nodes as text a patch may change. A verb's text is its schema and its program, which is code; a policy node does not exist yet. Trigger: the first `patch` refused for naming one.
...
- `@dbd/fabric` at [packages/fabric/src/index.ts](./packages/fabric/src/index.ts): the closed vocabularies (consequence classes, space kinds, decisions, change targets), the schema of record for spaces, verbs, receipts, reflections, crossings, bridges, weak references, and the event kinds; `project` and `apply` as the fold; `manifestFor` and `toolsFrom` as the projection; `fabricIssues` as the invariant check; `EventLog`, `GraphSource`, and `Consent` as Effect tags with an in-memory log and consent over any log.
...
- The verbs at [packages/fabric/src/verbs.ts](./packages/fabric/src/verbs.ts): `slice`, `reflect`, `patch`, `bridge`, `recall`, `pending`, `sync`, each a zod schema, a JSON Schema emitted from it, and an Effect program that may fail with a named reason; `refusal` as the gate a call passes; `decidePatch` and `decideBridge` as the operator's answers, never verbs; the `Canon` port with the on-disk skills behind it at [packages/fabric/src/node/canon.ts](./packages/fabric/src/node/canon.ts).
- The instrument at [packages/fabric/src/compounding.ts](./packages/fabric/src/compounding.ts): the pure fold from retrievals and later uses to R(t) and its companions; [packages/fabric/src/compounding.test.ts](./packages/fabric/src/compounding.test.ts): the four properties over generated logs (INV-FAB-005, INV-FAB-006, INV-FAB-011, and the portable format's identity through the file log), then a retrieval logged with its candidates, a use counted only when another session made the node, a session's own fresh reflection not counted, a citation nothing surfaced counted as missed, and INV-FAB-010 reported.
- The synthetic proof at [packages/fabric/src/sim/](./packages/fabric/src/sim/): the generative model, the θ-parameterized resonance, the harness that drives the real `slice` and `reflect` through `handleCall`, and the `import:synthetic` firewall; [packages/fabric/src/sim/quarantine.test.ts](./packages/fabric/src/sim/quarantine.test.ts) holds the firewall and the real log's clean R(t); [packages/fabric/src/sim/discrimination.test.ts](./packages/fabric/src/sim/discrimination.test.ts) holds that the metric climbs monotonically with planted quality and that `orient`'s k has no interior knee; `fabric/sim/baseline.json`, regenerated by `pnpm fabric sim-baseline`, is the §5 regression gate's anchor.
...
- The log at `fabric/spaces/`: two spaces opened, six verbs and three sources proposed, none blessed; every verb now asks for a `because`.
