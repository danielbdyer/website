...
- **Graph proximity in the fusion.** `slice` ranks by resonance then mention; the sky's presence ranks by strokes, shared facets, concordance, and distance; the charter asks for the two fused with a stated weighting. The weighting is unstated because there is no retrieval log to tune it against yet. Trigger: fifty retrievals in the log, when hit@k can tell one weighting from another.
