...
- **Warmed by intent, never by arrival.** A lazy layer may be fetched ahead when the visitor reaches for the surface it belongs to — the atmosphere on the Foyer's look-up pull's first input, or the "Look up" link under the pointer (`webgl/warmAtmosphere.ts`) — so the surface arrives whole. The warm-up is a fetch the visitor's own gesture asked for; it is never scheduled by a page's arrival, and it obeys the same gates.
