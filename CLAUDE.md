...
This file is the entry point — the foyer of the agentic surface. The full reading order lives in [SPECIFICATION_MAP.md](./SPECIFICATION_MAP.md#reading-order), but the path from here is: [MEDIUM.md](./MEDIUM.md) (what the medium is) → [TRANSPARENCY.md](./TRANSPARENCY.md) (the site publishes its own making) → [SPECIFICATION_MAP.md](./SPECIFICATION_MAP.md) (the map of all concerns) → the domain model → [DESIGN_SYSTEM.md](./DESIGN_SYSTEM.md) → [REACT_NORTH_STAR.md](./REACT_NORTH_STAR.md) (component architecture) → and outward from there. Each file is downstream of this one. When any specification and the felt sense of this place disagree, slow down. Listen for which one is more true. That slowness is not inefficiency. It's the practice.
