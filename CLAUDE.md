...
This file is the entry point — the foyer of the agentic surface. The full reading order lives in [SPECIFICATION_MAP.md](./SPECIFICATION_MAP.md#reading-order). The entry sequence is linear: this file → [MEDIUM.md](./MEDIUM.md) (what the medium is) → [TRANSPARENCY.md](./TRANSPARENCY.md) (the site publishes its own making) → [SPECIFICATION_MAP.md](./SPECIFICATION_MAP.md) (the map of all concerns). After that, the graph forks into two trunks: the semantic root ([DOMAIN_MODEL.md](./DOMAIN_MODEL.md) — rooms, facets, works) and the material root ([DESIGN_SYSTEM.md](./DESIGN_SYSTEM.md) — the visual language). [REACT_NORTH_STAR.md](./REACT_NORTH_STAR.md) sits at their confluence — where domain and design become code. Follow the trunk you need. Each file is downstream of this one. When any specification and the felt sense of this place disagree, slow down. Listen for which one is more true. That slowness is not inefficiency. It's the practice.
