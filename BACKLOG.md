...
**State:** Substantially shipped at `/sky`. The full structural future state in pure SVG/CSS is real: layered firmament with paper-grain noise, watercolor-filtered halos, vespers thread bloom, slow rotation, per-star twinkle, cursor parallax, the daystar, the polestar, and the carpet rolling out on first paint. `CONSTELLATION.md` §"What Shipped (First Form)" enumerates every shipped element. *What remains held* is the **WebGL atmospheric layer** specifically — capability SVG filters cannot deliver: cursor-responsive twinkle field, depth-aware shader parallax, drifting motes, procedural noise with continuous parallax offset. The held overscroll-up gesture, the daystar's cross-page morph via View Transitions, the spec strata layer, the time slider integration, audio in the Salon's region, and per-room sub-skies are each named in `CONSTELLATION_HORIZON.md` with their own triggers.
**Trigger to graduate to WebGL:** when the SVG layer's restraint reaches its ceiling (felt cost, or visible regression in filter pipeline performance with 8+ stars), OR when a visual capability the SVG filters cannot honestly deliver becomes the one the surface is asking for, OR Danny's pull. Performance plan (route-level code split for the WebGL bundle), accessibility plan (keyboard / reduced-motion / Save-Data fallback chain), and `ogl` (or hand-rolled) defense in the commit message are the doing-it-right register the WebGL move must arrive in.
