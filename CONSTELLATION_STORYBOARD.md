...
- **The seat** (the ninth pass and the tenth, `layout/DaystarSeat.tsx`, `dom/daystarSeat.ts`, `tokens.css` §"The daystar's seat", `routes/index.tsx`, `routes/sky.tsx`): the glyph is left alone at the start and falls with the page. On the way up a fixed seat outside the stage appears in the sky, at exactly where the sky seats its daystar and at the face's size, fading in from late in the pull to early in the lift (the glyph fading beneath), and takes the daystar's name, so the transition turns the moon into the face where it stands; on the way down the seat stands at the glyph's own rest from the first paint, so the face flies to exactly there and resolves into it, and the glyph has its place back once the transition has finished. The turn's curves mirrored to meet edge-on, its images crossing a breath either side of the edge. Tested in a browser on both projects: the seat absent until the reveal is past 0.6 and, at the transition's start, named, whole, and centered on the daystar within four pixels, the glyph unnamed; on the way down the seat's place the glyph's own within two pixels, the transition's flight ending exactly there, and the glyph named and whole once the seat has gone.
