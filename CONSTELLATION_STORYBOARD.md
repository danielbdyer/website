...
*Changes (the sixth pass).* Once the whirl has let go the scarf slips away over five seconds, unnoticed, and rests not drawn until the pointer's next visit; the body's paint turns on beneath. The heavens gain their gas clouds by night, and a star at rest is a cool pale point while the attended star is gold and half again as bright.

*Changes (the seventh pass).* By night the sphere no longer holds the heavens: the clouds fill the frame with a point of view, and the sphere is only where the stars are, its limb the horizon they slow toward. Dusk opens it; dawn collapses it onto the chart, which stays as it was, a little more parchment.

*Changes (the fifth pass).* The turn is a dissolve into the scarf: the silk keeps the setting hour's colors for the first quarter second and crosses to the rising hour's over the next 700 ms; the setting face stretches and blurs into the silk as it goes edge-on, the rising one condenses out of a blur, the painted body thins at the edge, and the whirl wraps the coin at the rim (§"The Fifth Pass").
...
*Changes (the fifth pass).* The glyph became the daystar as the room sees it — the crown by day, the back of the moon's head by night, the ascent turning it half round.

*Changes (the sixth pass).* The rooms keep their own two icons, pixel for pixel, on Danny's word; the half-turn stays, the far side of the moon turning away as the face comes round. The ascent is the lift, and the sky is readied while the Foyer rests, so the lift plays over a sky that is simply there.

*Changes (the seventh and eighth passes).* The strip of sky that came down from the top as the visitor pulled — *a garage door*, Danny said — is gone; so is the flat page that swung away in its place — *a garage door opening the other way*; and so are the corners of the page turning about the eye, which Danny circled: *the 3D space continues to exist below; the homepage keeps the full width while it falls away, correctly.* Looking up is the eye turning back in a space that stays: the room is a surface continuing past the frame on both sides and below, foreshortened as it falls from view, its top edge the one edge kept; the heavens are a dome behind it — the very canvas the readiness painted — turned by the same pitch on the same frame, from the sky's rest view turned down by the whole lift to that rest view exactly, and only then does the route change. The descent is the same arc in reverse on the same canvas, handed back rather than disposed.
...
- **The seat** (the ninth pass to the eleventh, `layout/DaystarSeat.tsx`, `dom/daystarSeat.ts`, `tokens.css` §"The daystar's seat", `routes/index.tsx`, `routes/sky.tsx`): the glyph is left alone at the start and falls with the page. On the way up a fixed seat outside the stage holds the character itself — the sky's own daystar molecule, lazily fetched, cached by the readiness — at exactly where and how large the sky seats it, appearing as soon as the falling page has uncovered that place and whole early in the lift, by night turning in from the edge; the character carries the daystar's name, so the transition crosses face to face, and the glyph fades in the falling page at its own size with a breath of growth; on the way down the seat stands at the glyph's own rest from the first paint, so the face flies to exactly there and resolves into it, and the glyph has its place back once the transition has finished. The transition's half-turn belongs to the descent alone, its curves mirrored to meet edge-on, its images crossing a breath either side of the edge. Tested in a browser on both projects: the seat absent until the reveal is past 0.6 and, at the transition's start, the character named, whole, and the daystar's own box within four pixels, the glyph unnamed and faded; no turn in the ascent's transition and the half-turn in the descent's; on the way down the seat's place the glyph's own within two pixels, the transition's flight ending exactly there, and the glyph named and whole once the seat has gone.
...
## The Fifth Pass — The Glyph's Identity and the Dissolve

On Danny's next three pulls the same evening — *the moon flips around and does a 180 to show the other side, visibly transitioning at the same time into the character; the svg rays of the sun glyph should maintain their identity as they transform into the primary rays for the sky version; the characters morph into the scarf — partially dissolving into it, preserving the first half's identity through the transition and ending with the second half's: the moon-sun-scarf is the character, holarchically.* Enforced in code:

- **The glyph** (`atoms/DaystarGlyph`, `app/layout/ThemeToggle.tsx`): the sun as the crown itself with a wash of a disc; the moon as the back of the head (`faceGeometry.ts: MOON_BACK` — the disc's left rim and an arc bulging left, horn to horn); drawn in the daystar's square. Tested as anatomy, and in the toggle: the crown by day, the back by night.
- **One clock** (`hooks/useCrownPhase.ts`): every crown under a root takes the wall clock's phase on mount — the toggle's and the daystar's — so the rays keep their bearings across the morph. Tested: the phase as a value; every crown under a root set alike; and, in a browser, the sky's crown within seconds of the glyph's after a look-up.
- **The half-turn** (`tokens.css` §"The ascent and the descent"): `html.dk::view-transition-old(daystar)` turns away to 90° and `::view-transition-new(daystar)` comes round from −90°, with perspective, over the morph's 900 ms; the descent's delays restate themselves after. Tested in a browser: the two pseudo-elements carrying `rotateY` keyframes through a night-time look-up.
- **The dissolve** (`tokens.css` §"The daystar", `dom/daystarMagic.ts`): `--silk-1…4` registered with `@property` and transitioned 700 ms after a 250 ms wait, restated inside the theme window; the setting face's `scale(1.1, 0.02)` and `blur(3px)`; the body's half-opacity at the edge; the whirl's radius to the rim. Tested in a browser: the silk's first stop still the day's color through the first 180 ms and no longer by 1.3 s.

## The Sixth Pass — The Glyphs Kept, the Scarf's Leaving, the Lift, the Heavens (and the Seventh — the Heavens Opened)

On Danny's word after the fifth pass: keep *precisely* the sun and moon glyphs the rooms had, and have the daystar transition back into them; the world canvas should be pre-mounted so the look-up has no jank; the scarf should fade after the turn without being seen to leave, and not return until the next hover; and the thing he always wanted from the transition — the skill-sky of a certain northern game: swirling gas clouds, quiet and attended told apart clearly, and the feeling of the menu lifting to look into the heavens, the neck going up as the chin comes out, the camera pitching back as if about to be slung skyward. Enforced in code:

- **The glyphs** (`app/layout/ThemeToggle.tsx`): restored from the commit before the fifth pass, byte for byte — the diff against it is empty. `atoms/DaystarGlyph` is gone; the half-turn on the ascent stays; the sky's crown keeps the wall clock's phase.
- **The scarf's leaving** (`dom/daystarMagic.ts`, `--scarf-presence`): a presence mood, tweened to nothing over five seconds after the whirl, back to one on hover; the ticker draws no scarf while it is gone. Tested on a fast clock in the driver, and in a browser across a real turn.
- **The lift** (`tokens.css` §"The look-up", `dom/lookUp.ts`, `routes/index.tsx`, `routes/__root.tsx`, `hooks/useThresholdReveal.ts`, `webgl/readySky.ts`): the page is one space on a stage — `.site-stage` holds the eye's perspective while a pull gathers, and `.site-room` turns about a point one focal length before it, its surface continued past the frame by a pseudo-element so no corner ever shows, its nav, main, and footer at small depths that grow with the pull. `dom/lookUp.ts` holds the numbers the room and the heavens share — thirteen degrees per unit of `--reveal`, the lift's end at 3.7 — and carries the lift from the pull's one to that end over 900 ms on the signature curve, writing the room's turn and the heavens' pitch on the same frame; the readiness paints its canvas as a dome seen at the Foyer's rest pitch and registers how to turn it, the pull's hook reports each written reveal, and the route changes only when the lift has landed. On the way down the sky's mount hands its atmosphere off instead of disposing it (`hooks/useWebGLFirmament.ts`, `webgl/warmAtmosphere.ts` `holdAtmosphere`), the readiness sets it back into the backdrop at the room's current reveal, and the Foyer's settle-in turns it down from the lift's end to rest before the first paint; the gesture hook's unmount leaves a descending root alone. Tested in a browser on a manual clock: frames of the lift and of the descent, the heavens coming down through the frame at the rate the page falls away, the same canvas in the backdrop, the sky's frame, and the backdrop again.
- **The seat** (the ninth pass, `layout/DaystarSeat.tsx`, `dom/daystarSeat.ts`, `tokens.css` §"The daystar's seat", `routes/index.tsx`, `routes/sky.tsx`): the nav's glyph gives the daystar's name to a fixed seat outside the stage the moment the eye moves — the same icon at the icon's own rest, the glyph hidden beneath — which rises through the lift to exactly where the sky seats its daystar, so the transition turns the moon in place; on the way down the seat begins where the daystar stood and comes to the corner once the transition has finished. The turn's curves mirrored to meet edge-on, its images crossing a breath either side of the edge. Tested in a browser on both projects: the seat named and centered on the daystar within four pixels at the transition's start, the glyph unnamed and hidden; the seat's first place on the way down the daystar's; the glyph's place its own again once the seat has landed.
- **The readiness** (`hooks/useSkyReadiness.ts`, `webgl/readySky.ts`, `webgl/warmAtmosphere.ts`): at the Foyer's idle, the route preloaded, the graph fetched, the atmosphere prepared for its scene — context and programs — painted once at rest and set into the backdrop, and the magic warmed; `mountAtmosphere` adopts a prepared atmosphere for the same scene, set to the hour's tones, its painted frame kept (a size already held is not set again), the frame claimed at once without a crossfade. Tested: the scene key, one preparation per scene, adoption once and only for its own scene; and in a browser, the route's, the renderer's, and the magic's chunks fetched on the Foyer with no gesture at all, the prepared canvas in the backdrop, and the same canvas in the sky's frame after the look-up.
- **The heavens** (`webgl/atmosphereShaders.ts`): the gas clouds in the dome's dark hour — banks from the wash folded by the mass, filaments from one more warped read, teal and green with gold and a breath of rose — and the glow's quiet-versus-attended: cool pale points at rest, gold and white and brighter for the star that has the visitor's attention; the drawn gold ring fuller on a claim.
- **The heavens opened** (the seventh pass, `webgl/atmosphereShaders.ts`): by night no page beyond the plate — the sky read off the ray's own direction past the limb, blended in over a band inside the rim; the clouds given a point of view by one world-anchored sweep; the limb's gather, the rim's line, and the vignette fading with the opening; the chart's lines kept to the plate. By day the plate, the page, and the render as they were, nudged toward parchment: a slow mottle in the sheet and a warmer edge. The opening and the collapse follow the dusk.
- **The heavens' palette** (the eighth pass, `webgl/atmosphereShaders.ts`): the teal-and-green banks — *algae*, Danny said — replaced, against three Hubble plates and the design's own write-ups, by a pale blue-white haze folded into banks, ridged filaments white and ochre, rust where the mass is densest, and lanes of dark warm dust with a lit rim; a fourth, finer star layer gathered inside the cloud and reddened in its dust; the sweep's far side deeper; the compass's atmospheres read off the ray by night so no plate edge shows. Three rounds by eye, each rendered and looked at: the first too full and too black in its lanes, the second quieter, the third truer in its rust.
...
- **The face's held marks.** A second, inner crown of short rays, as the clocks' suns wear; whether the scarf wants to trail sparks through the turn, or the moon's crescent to face the sun across it; the sparks' count and reach; whether the turn wants a sound; the paint's flow and shimmer, its tones by hour, and how far the drawn discs should thin over it; the dissolve's true key frames — the rays streaming into the strands, the crescent gathering out of them — beyond the blur, the stretch, and the silk's late crossing; the half-turn's perspective and its timing against the morph. The card's beard and nightcap were set down with the card's register. Each is a stroke in `DaystarFace` or `scarfGeometry`, a mood in the driver, a line in the painter's shader, or a line in the register, for Danny's eye.
- **The lift's angles and the clouds' colors.** The room's tilt and drop, the sky's approach, the clouds' banks and filaments and how far they should stay by day, the quiet star's paleness and the attended star's gold — each set once, in a headless browser, against Danny's memory of the skill-sky. For his eye on the lived look-up.
- **The glyph's identity, held again.** The crown carried into the sky's crown, the moon's back turning to its face, were built and set down for the rooms' own icons; if the rooms' glyphs ever want to become the character's, the fifth pass's commit has the drawing.
