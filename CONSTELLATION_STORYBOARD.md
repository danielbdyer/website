...
*Changes (the sixth pass).* The rooms keep their own two icons, pixel for pixel, on Danny's word; the half-turn stays, the far side of the moon turning away as the face comes round. The ascent is the lift: the room drops away beneath and tilts, its top toward the eye, while the sky comes down from above, larger and tilted the other way, to meet the gaze — one camera move, 900 ms. And the sky is readied while the Foyer rests, so the lift plays over a sky that is simply there.
...
- **The lift** (`tokens.css` §"The ascent", `routes/index.tsx`): `html.ascending` for the look-up's duration; the room away (translate, `rotateX(-26deg)`, a little smaller, fading) and the sky down (from larger, `rotateX(16deg)`, above) over 900 ms on the signature curve. Tested in a browser: the class seen and released across a look-up.
- **The readiness** (`hooks/useSkyReadiness.ts`, `webgl/readySky.ts`, `webgl/warmAtmosphere.ts`): at the Foyer's idle, the route preloaded, the graph fetched, the atmosphere prepared for its scene — context and programs on a canvas not yet on the page — and the magic warmed; `mountAtmosphere` adopts a prepared atmosphere for the same scene, set to the hour's tones, and skips the arrival wait. Tested: the scene key, one preparation per scene, adoption once and only for its own scene; and in a browser, the route's, the renderer's, and the magic's chunks fetched on the Foyer with no gesture at all.
