...
*Changes (the seventh pass).* The strip of sky that came down from the top as the visitor pulled — *a garage door*, Danny said — is gone. The sky is a fixed backdrop under everything, holding the very canvas the readiness painted; the pull tilts the room away beneath the gaze over it; the commit continues the tilt on the room's own image while the backdrop crossfades into the sky it already was, and the sky adopts that same canvas whole, claimed at once. The descent is the lean-back undone.
...
- **The lift** (`tokens.css` §"The look-up", §"The ascent", `routes/index.tsx`, `routes/__root.tsx`, `hooks/useThresholdReveal.ts`): the page is one room (`.site-room`); a gathering pull marks the root (`html.pulling`) and tilts the room over the backdrop (`--reveal` → a perspective, a drop, a pitch), untouched at rest; `html.ascending` for the look-up's duration names the room for the transition, and its old image falls away (`translateY(46%) rotateX(34deg)`, fading late) over 900 ms while the root crossfades. Tested in a browser: the class seen and released across a look-up; a short pull tilting the room and a release settling it.
