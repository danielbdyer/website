...
**The lift (the sixth pass).** Looking up is a lean-back into the heavens. Danny's memory of the skill-sky in a certain northern game: the menu lifting to look *into* the heavens where the constellations are, an appeal to the gods; as the neck goes up the chin comes out, the camera pitching back as if about to be slung skyward, or sitting in a rocket at the moment before it goes; and it works because you know the character is looking forward — you are natively oriented — so a looking-up can actually read as looking up. The ascent is now one camera move across both pages rather than a crossfade: the room drops away beneath and tilts, its top coming toward the eye as it goes (`perspective` and `rotateX`, 900 ms on the signature curve), while the sky comes down from above, larger and tilted the other way, and settles to meet the gaze. The Foyer names it on the root (`html.ascending`) for the look-up's duration; the daystar's morph rides its own group over the same arc. The earlier pitch that folded the *destination* like a sheet of paper is not repeated: the room and the sky move as one scene, and neither is bent alone.

**The sky, readied (the sixth pass).** *It should be pre-mounted somehow already, so that there's no jank.* While the Foyer rests — after load, once the browser is idle — everything the sky mounts is made ahead (`hooks/useSkyReadiness.ts`, `webgl/readySky.ts`): the sky route's code and its loader's graph; the atmosphere's module, then its context and its compiled programs, on a canvas not yet on the page, prepared for the graph's scene and adopted whole when the sky mounts (`webgl/warmAtmosphere.ts`: `prepareAtmosphere`, `adoptAtmosphere`); and the daystar's magic. Nothing is created under the visitor's eye, so the lift plays over a sky that is simply there. This is the Foyer's own readiness — the look-up is its primary gesture and its ceiling is the sky — paid at idle and never on arrival, under the same gates.
