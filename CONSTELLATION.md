...
- **The rest of the atmospheric WebGL layer.** Pass 2 Phase E shipped the cursor-following luminous pool. The next pass added per-star halos with breathing twinkle (the held intent the SVG `star-twinkle` keyframe could no longer carry honestly), a polestar wash that swells on a slow tidal cycle (~14s), and four drifting motes on slow Lissajous paths — all painted by the same fragment shader against the post-rotation star positions the projector now broadcasts each frame. The atmospheric layer is alive without taxing the rasterizer. *What still pulls forward:* depth-aware parallax via shaders (multiple atmospheric layers at different parallax depths), brushstroke threads in motion (the held filter can return as a paint pass that doesn't re-rasterize), cursor-responsive twinkle modulation (twinkle intensity reading the cursor's proximity). The held vision in `CONSTELLATION_HORIZON.md` Phases 2–3 names what WebGL uniquely makes possible. Trigger: when a visual capability the current shader cannot deliver becomes the one the surface is asking for.
