...
**The glyph's identity (the fifth pass, 2026-09-05, with Danny).** The nav's glyph is the daystar as the room sees it (`atoms/DaystarGlyph`), drawn in the daystar's own square so the look-up's morph is between like and like. By day it is the sun's crown of rays itself, in the page's ink, with a wash of a disc — and every crown on the page keeps one clock (`hooks/useCrownPhase.ts`: the animation's phase set from the wall clock on mount), so the rays keep their bearings from the corner into the sky; the morph carries the rays themselves into the sky's crown, as Danny asked, rather than one sun fading into another. By night it is the back of the moon's head — a plain crescent lit on the left (the room's moon is waning; the sky's, lit on the right with its face turned left, is waxing — the two sides of one head) — and the ascent turns it half round: the back turns away as the face comes round the other side, over the morph's 900 ms with a little perspective; the descent turns it back. The turn plays on the view transition's old and new images (`html.dk::view-transition-old/new(daystar)`), so it costs nothing and needs no script.
