...
~~A held question: does the icon's *form* change as it ascends?~~ **Resolved (2026-09-05, with Danny): yes, and further than a corona.** The celestial form is the small form *grown into its place* — grown a face. The first drawing that day was a card's: plump and rosy, a bulb of a nose, a jovial mouth, the mid-century Christmas card's Santa. Danny's self-critique the same afternoon — *less clowny, more illustrated; watercolored, wispy; Destino and Fantasia; the sun and moon clocks* — moved the register to the engraver's. The sun is now the sun in splendour from an astronomical clock's dial: a crown of sixteen rays, straight and wavy by turns, each cut a little differently, turning once every two minutes and twenty; the dial's fine rings inside the rim; thin arched brows, almond eyes with a lid line and a lash line, a single line of a nose, a closed smile; faint cheeks; watercolor granulation settled into the gold. The moon is the crescent asleep in profile at the sun's shoulder — the picture book's moon — its inner edge a face turned left with its eye closed, the earthshine of the whole disc faint in its hollow with three small stars, freckles of crater on the lit body. The face breathes, blinks, turns a degree toward the pointer, and its eyes follow. The physics is a watercolor's: every wash a gradient with a wet rim, the line-work one thin ink in the hour's own color, the still filters on the halo and the grain. *The clay of life with an illusory veneer.* Danny's reference images (a card's Santa; the sun with the sleeping moon at its shoulder) were inspiration, not specification; the outcome is the site's own.

**The magic.** Around the face a scarf of silk swoops in three dimensions — behind the disc, out through the rays, over the face — in the hour's four colors, which sweep along it; three strands, the main one and two wisps that part from it like smoke; a sheen along the crest. At rest it drifts; under the pointer it quickens and brightens; through the turn it whirls tight and lets go. It is a lazy layer (`PERFORMANCE_BUDGET.md` §"The Sky's Lazy Layers"): its geometry is pure (`sky/scarfGeometry.ts`), its driver (`dom/daystarMagic.ts`) tweens the scarf's moods with GSAP and writes the paths on GSAP's ticker, and the driver and the library are fetched together only after the page has loaded and gone idle — never ahead of the sky's first paint, never under reduced motion or Save-Data, and switched off by `?magic=off`. The molecule keeps two empty slots for the scarf, behind and in front; an authored animation, when one exists, goes in the same slots on the same path (`BACKLOG.md` §"An authored animation layer over the daystar").

**The daystar is the toggle.** Clicking the face turns the hour. The turn is a coin's, and a setting: the setting face sinks a little as it goes edge-on over 440 ms, and the rising face comes round the other side from just below over the next 500 ms, after a 300 ms wait — the moon was behind the sun all along — with a flare and eight sparks out of the turn, 0.8 s in all. (The first cut ran 1.4 s; Danny saw that the segment from a half to two thirds through could go and no one would notice, and it went.) Both faces are always in the markup; the html's hour chooses, so first paint is right before any script. Reduced motion swaps the hour at once and holds the face still.

**The light (the fourth pass, 2026-09-05, with Danny).** *Show a sunset because of the sun; real light shading to give legibility to the reality of the situation.* The faces are lit bodies, not stickers. Drawn, the sun is its own light — a core brightening toward the upper left, a limb that warms and darkens toward the rim — and the moon is lit from the right, where its rim is, a terminator shading the crescent toward the profile and the earthshine dim in its hollow; the silk that crosses the sun is backlit where it passes over the disc, and the silk that crosses the moon casts a soft shadow on the lit body — two echoes of the scarf's front strand, written by the magic and clipped to the body each lights or shadows. Painted, where WebGL is to be had, the body is a small canvas of the daystar's own between the scarf's two slots (`webgl/daystarPaint.ts`): a lit sphere of living pigment beneath the ink — a warped field that flows, swirls that repaint as the body turns, a shimmer of rainbow in the swirls that fills under the pointer and through the turn, the way the fish in Danny's reference shimmer as they swim; the drawn discs thin to a wash and the grain and the line-work ride on top. The body turns with the coin, edge-on and round again, and its paint crosses into the other hour while it is edge-on. The painter is part of the magic's lazy layer and mounts with it; until then, and wherever there is no WebGL, the drawn discs are the body.

And the sky answers the sun: when the hour turns, the frame carries its dusk (`data-dusk`) for the atmosphere's 1.8 s arc, and a sunset gathers at the daystar's seat — gold nearest the sun, rose beyond, violet at the far edge, a flush along the foot of the frame, fullest not quite halfway, gone at both ends — while the daystar's own light flares rose-gold behind the faces. The sunset is one surface in the frame (`.constellation-dusk`, keyed to the seat the projector writes on the frame), so it belongs to both substrates: the chart without the weather sets its sun too, and the WebGL atmosphere's own horizon flush plays beneath it on the same arc.

**The ascent.** Looking up from the Foyer, the nav's glyph readies itself with the pull — it lifts and grows as the sky leans in — and on commit the View Transitions API carries it up into the daystar's face on its way to the sky's margin, over a held second on the signature curve; the face then makes a small entrance, as a character who was always there. The sky's atmosphere is warmed as the visitor reaches for it — the first input of the pull, or the pointer resting on the link (`warmAtmosphere`, `hooks/useWebGLFirmament.ts`) — so a committed look-up arrives already lit, in one substrate, rather than the chart first and the weather a breath later; a warmed atmosphere mounts the moment the sky does.

**The glyph (the fifth pass and the sixth, 2026-09-05, with Danny).** For one pass the nav's glyph became the daystar as the room sees it — the crown itself by day, the back of the moon's head by night. Danny's word the same evening: keep *precisely* the sun and moon glyphs the rooms had, modern and clean, and have the daystar transition back into exactly them. So the rooms keep their own two icons (`atoms/SunIcon`, `atoms/MoonIcon`, pixel for pixel as before), and the morph carries the daystar's face into them and out of them. What stays from the pass: by night the glyph reads as the far side of the moon — the room's crescent is lit on the left, waning; the sky's, lit on the right with its face turned left, is waxing — and the ascent turns it half round, the far side turning away as the face comes round the other side, over the morph's 900 ms with a little perspective; the descent turns it back. The turn plays on the view transition's old and new images (`html.dk::view-transition-old/new(daystar)`), so it costs nothing and needs no script. The sky's crown keeps the wall clock's phase on mount (`hooks/useCrownPhase.ts`), so every look-up finds it where it would have been rather than back at its first bearing.

**The turn as a dissolve (the fifth pass).** *The moon-sun-scarf is the character, holarchically: preserve the first half's identity through the transition, and end with the second half's.* The silk's four colors are registered properties, so the scarf's hues transition rather than switch: through the turn the scarf keeps the setting hour's colors for the first quarter second and crosses to the rising hour's over the next 700 ms — the setting face's identity carried in the silk while it goes, the rising face's arriving after. The setting face stretches and blurs as it goes edge-on, into the silk; the rising face condenses out of a blur; the painted body thins to half at the edge; and the whirl wraps the coin at the disc's rim, longer and closer, so the face goes *into* the scarf and the other comes *out* of it. A first form: the key frames of a true dissolve — the rays streaming into the strands, the crescent gathering out of them — are held for a pass with Danny's eye on the lived turn.

**The scarf's leaving (the sixth pass).** Once the turn's whirl has let go, the scarf slips away — over five seconds, too slowly to be seen going — and rests, not drawn, until the pointer's next visit calls it back; the body's paint keeps turning beneath. *It should eventually fade away without the user noticing that they leave, and not come back until the user hovers again.* A presence in the driver's moods, handed to the register as `--scarf-presence`, which every strand and both echoes multiply.

**The lift (the sixth pass, and the seventh).** Looking up is a lean-back into the heavens. Danny's memory of the skill-sky in a certain northern game: the menu lifting to look *into* the heavens where the constellations are, an appeal to the gods; as the neck goes up the chin comes out, the camera pitching back as if about to be slung skyward, or sitting in a rocket at the moment before it goes; and it works because you know the character is looking forward — you are natively oriented — so a looking-up can actually read as looking up. The first cut lowered a strip of sky from the top of the viewport as the visitor pulled; Danny's verdict — *it looks like someone's garage door coming down* — was exact, and the strip is gone. The sky never moves in now. It is a fixed backdrop under everything, hidden while the room stands and holding the very sky the readiness painted ahead; and the pull tilts the *room*: the page, one body, drops away beneath the gaze and its top recedes (`--reveal` on the root driving a perspective and a pitch on the room only while the pull gathers, so the room at rest is untouched), and behind it the heavens are simply there, as they always were. The commit continues the same move as a view transition on the room's own image (`html.ascending`, the room named for the transition's duration): the room falls away and fades while the backdrop crossfades into the sky it already was, and the daystar's morph rides its own group over the same 900 ms. The descent is the lean-back undone: the sky drifts up and out and the room rises back into place beneath the gaze. Reduced motion never writes the pull, and the transitions hold still.

**The sky, readied (the sixth pass, and the seventh).** *It should be pre-mounted somehow already, so that there's no jank.* While the Foyer rests — after load, once the browser is idle — everything the sky mounts is made ahead (`hooks/useSkyReadiness.ts`, `webgl/readySky.ts`): the sky route's code and its loader's graph; the atmosphere's module, then its context and its compiled programs, prepared for the graph's scene, painted once at rest, and set into the backdrop behind the room — so the pull reveals the sky itself, not a stand-in — and adopted whole when the sky mounts (`webgl/warmAtmosphere.ts`: `prepareAtmosphere`, `adoptAtmosphere`): the same canvas moves from the backdrop into the sky's frame, its painted frame kept, the frame claimed at once with no crossfade; and the daystar's magic. Nothing is created under the visitor's eye, so the lift plays over a sky that is simply there. This is the Foyer's own readiness — the look-up is its primary gesture and its ceiling is the sky — paid at idle and never on arrival, under the same gates.

**The heavens (the sixth pass, and the seventh).** *Swirling gas-cloud backgrounds; clear yet unassuming passive/active color distinctions.* The dome's dark hour gained the gas clouds: banks of teal and green — the wash folded by the mass — with filaments of gold from one more warped read, finer, on its own slow clock, and a breath of rose where the mass gathers; brightest where the banks fold and toward the zenith, thinning to the deep. And the glow pass tells quiet from attended without fuss: a star at rest is a cool, pale point with its facet's hue only as a tint; the star that has the visitor's attention is gold, white at its core, and half again as bright; the drawn gold ring answers a claim a touch fuller than before.

Then Danny's next word: *I don't know that we need the sphere as a container in the dark.* So by night the heavens are not held in the sphere. They fill the frame — with a point of view: one great sweep of the sky carries the banks and the far side stays deep and dark, so the clouds compose rather than fill — and the sphere is only where the stars are, its limb the horizon they slow toward as they turn (the stars keep their curved sky, and near its edge they compress the way things do at an horizon, though the eye never moves). Beyond the limb the sky is read off the ray's own direction, a dome around the eye, blended in over a band just inside the rim so nothing seams; the limb's gather and the rim's line fade with the opening, and the frame's vignette lets the heavens keep more of themselves. By day the same sky *collapses onto the chart* — Danny's phrase, and his decision that this is what the day is: the sphere is the plate again, the page outside it, the chart's rings and meridians on the plate only, the clouds a trace on the sheet and nowhere else — and the render is otherwise exactly as it was, nudged a little toward parchment: a soft unevenness in the sheet from one slow noise, warmer where the paper is thicker, and an edge a shade warmer, like a page kept a long time. Not much at all. The opening and the collapse ride the dusk's own 1.8 s. `CONSTELLATION_HORIZON.md` §"Layer 1" describes the atmosphere this extends.

**The way down (the fourth pass).** Looking down — the return pull, ArrowDown, Escape, the link — continues the pull rather than crossfading: the room slides up beneath from where its ground had leaned in, while the sky drifts up and out; the daystar stays exactly where it is through the reveal, and only when the room has settled does it travel down into the nav's corner and become the glyph. The sky route names the descent on the root (`html.descending`) for the return's duration; the choreography is CSS on that class. *Have the sun or moon stay in place as the page reveals itself* — Danny's ask, kept literally. For the morph to read as one body, the nav's glyph now shows the hour the room keeps — the sun by day, the moon by night — rather than the hour a click would bring; the label still says what a click does. The daystar lives beside the sky's svg, not inside it, because a view transition names boxes, not strokes, and because a button belongs on the page. Held: whether the glyph wants a hint of the face before it rises — set down, mostly, by the fifth pass: the glyph is the crown and the back of the head, which is the hint.

**Held — one substrate.** Danny's tremor, 2026-09-05: *it should feel like the room app and the sky app are one and the same — already being in WebGL mode somehow, so that way it's not abrupt, and we can have travel of the glyph/character as active in both substrates. Maybe the whole site is canvas?* And, with a recording of a painting in motion — brushwork that evolves continuously, fish shimmering with swirling color as they swim: *a shaded texture of paint in motion, where the watercolor scene is evolving in 2D space continuously as a mesh on an animated 3D model.* What is alive in it: the continuity of light and of the character across the boundary, and the sky as one painting repainted every frame rather than a chart with weather behind it. What this pass built of it: the atmosphere warmed ahead of the look-up; the daystar as the constant of the descent; the body painted as living pigment in a canvas of its own, which is the seed — a character that carries its own paint can carry it anywhere, into the nav's corner included. What is held: the room's prose stays DOM (selection, print, assistive output; the paper is the paper); a persistent WebGL context on every page is weight on arrival, which the budget refuses; the sky's ground as brushwork in motion — directional strokes, an evolving field the weather already half is — wants a pass of its own with Danny's eye on it; and the glyph in the nav carrying the daystar's paint, so the character is continuous rather than morphed, is the next smallest step. *Trigger:* Danny's word after living with the painted body and the descent.
...
| **Hover a bearing in the whisper, or a name at the rim** | The facet's whole figure lights | *The thread of you, drawn across the sky* |
| **Click a name at the rim** | Travel along that facet to the nearest star that carries it | *Taking a bearing from the compass itself* |
...
- ~~**The daystar's cross-page morph.**~~ **Shipped (2026-09-05).** The nav's glyph ascends into the daystar's face across the look-up and descends across the return (§"The Sun and the Moon"), and the daystar is the hour's toggle in the sky.
