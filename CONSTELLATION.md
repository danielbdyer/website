...
*Resolved 2026-06-13, from a conversation with Danny. The daylight mode's identity is a **reflecting pool**. Night and day are one sky seen from two directions: at night the visitor looks **up** into the firmament; by day they look **down** into still water that holds it. The water is the reason the daylight mode was always reaching for — the "watercolor ocean" was a wash without a subject, lovely and unaccountable; the pool gives day a point of view as old and as knowing as the night sky's. The decision was made first; the **rendering shipped 2026-06-13** in the WebGL dome's day branch (`atmosphereShaders.ts`, gated by `1 - uNight`). The pale flat wash became a still umber pool: the day sky is deepened into the body of water (water is darker than the sky it holds, which gives reflected light room to glint), mottled with pigment pooling where the weather gathers, bent by a slow refraction (crests brighten, troughs darken), with the umber bed gathering through the near water toward the bottom — and the daystar doubled on the surface as a shimmering glade falling toward the viewer, broken into glitter. Umber throughout, not blue; calm, not turbulent. The 500ms theme fade crossfades the two ontologies (firmament ↔ pool) through `uNight`. What remains held: a literal *waterline* edge and per-star reflection doubling (the watercolor points already soften like light on water; an explicit mirrored double is a later refinement).*
