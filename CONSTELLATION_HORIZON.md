...
*Pass 2 (Phase E) shipped the first form of this layer: a `<canvas>` mounted behind the structural SVG, an `ogl`-flavored render loop, and a fragment shader that paints a luminous pool of attention around the cursor's projected sphere position with a squared-falloff profile and a saturation boost in the active well's zone. A subsequent pass extended the shader with per-star halos that breathe on per-star twinkle phases (the SVG `star-twinkle` keyframe's held intent, now in a medium that doesn't re-rasterize), a polestar wash that swells on a slow tidal cycle, four drifting motes on slow Lissajous paths, theme-aware tone via a `uTheme` mix, an aspect-ratio uniform so circular falloffs render as circles on non-square viewports, and an `atmosphericScene` shared signal that the projector writes post-rotation positions into each frame so the shader avoids per-pixel trig. What still pulls forward: depth-aware parallax via multiple shader layers (today's pass is a single-layer atmosphere); brushstroke threads as a WebGL paint pass over their SVG navigable counterparts; cursor-responsive twinkle modulation (twinkle intensity sensitive to cursor proximity). The day↔night shader crossfade remains a `uTheme` mix rather than two compiled programs cross-faded — sufficient until a sample shows the seam.*
...
### Phase 2 (shipped — SVG + WebGL atmospheric pass) — *Does the sky have weather?*

Paper grain, layered radial gradient (sky-glow → zenith → horizon), watercolor-filtered halos on stars and the daystar — all shipped via SVG `<feTurbulence>`, `<feGaussianBlur>`, `<feDisplacementMap>` filter primitives + `mix-blend-mode: soft-light`. The atmospheric WebGL pass then layered on top: continuous procedural noise drifting slowly, the cursor-following luminous pool (Phase E), per-star halos painted at the projector's broadcast positions, breathing twinkle keyed to per-star phase, polestar wash on a slow tidal cycle, drifting motes. The sky has weather, and weather has weather. *What remains held*: depth-aware shader parallax (today's atmospheric pass is a single layer), cursor-responsive twinkle modulation (today's twinkle is per-star phase only), brushstroke threads as a WebGL paint pass.

### Phase 3 (shipped in SVG; WebGL still ahead) — *Does the sky live?*

Slow rotation (600s/cycle around the polestar), per-star twinkle (4.5s with deterministic phase offsets), cursor-driven parallax (two depths, signature easing) — all shipped via CSS animations + the `useConstellationParallax` hook. The Phase 2 WebGL pass added the per-star breathing twinkle the SVG `star-twinkle` keyframe couldn't carry honestly, plus the polestar wash breath and the drifting motes' slow lives. The sky lives. *What remains held*: parallax with real depth-of-field (today is two CSS layers); cursor-responsive twinkle that brightens near the visitor's gaze.
