...
*Pass 2 (Phase E) shipped the first form of this layer: a `<canvas>` mounted behind the structural SVG and a fragment shader painting a luminous pool of attention at the cursor's projected sphere position. Pass 3 (2026-06-12) shipped the full form, and it went further than this section first imagined: rather than a screen-space gradient with parallax offsets, the dome casts a per-pixel **view ray through the live navigation camera** (shared via a module-level camera signal the navigation hook broadcasts each tick) and intersects the latent sphere — so the painted sky is world-anchored and parallaxes with the visitor's travel for free. Three passes compose: the dome (gradient, domain-warped fbm weather, stereographic-charted micro-starfield, room-quadrant tints, daystar glow, cursor pool, luminance-aware paper grain), instanced star-halo sprites (pigment bleeds by day / twinkling glows by night, blended through `uNight`), and instanced motes drifting on shells above the sphere. Halo registration with the structural stars is exact: the hook replays the SVG's transform stack (cursor parallax → camera yaw) and the viewbox fit each frame, measured against the SVG's own layout box; the heavens' 600s turn is a roll carried by the shared camera itself (since 2026-09-01), so both renderers see it through the same basis rather than one replaying the other's animation. The palette is read from the live design tokens at init and on theme toggle — tokens.css stays the single source of truth — and crossfades CPU-side over the 500ms theme transition. When WebGL paints, the frame carries `data-atmosphere="webgl"` and the SVG firmament crossfades out; every fallback gate (no WebGL, context loss, Save-Data, forced colors, `prefers-contrast: more`) returns it. Reduced motion holds a still frame, repainting only on camera snap / theme flip / resize. A frame-budget watcher lowers the render resolution once if the device can't hold 60fps — the shader simplifies before anything structural does.*

*A performance-and-deepening pass followed in the same season. The frame path lost every per-frame `getComputedStyle` (the transform chain is replayed numerically from inline variables — and, since the heavens' turn moved into the camera, there is no rotation clock to read back), every steady-state allocation, and the structural projector's per-tick `querySelector` walks (now cached per camera group); the loop halves its cadence when the sky is calm, the context is opaque and depthless with no clear pass, the noise budget dropped from eight simplex calls per pixel to five, and shader compiles stagger one-per-frame at mount. The same pass deepened the paint toward the celestial-chart register CONSTELLATION_DESIGN.md names — a milky vein of clustered stardust on its own great circle, three depths of deep-field dust, hairline gold rings of constant polar angle breathing around the polestar on the 30s cycle the SVG wash once held, a luminous horizon gather with watercolor weather breathing through it, four-point sparkle on the brighter halos, and the dark hour's zenith tuned to the deep night-blue the design doc commits to. PERFORMANCE_BUDGET.md §"Monitoring" records the frame discipline and its protective floors.*
...
The look-up has shipped — not as the carpet/fold it was first imagined, and not (yet) as a moving camera. Two moves replaced the fold on 2026-06-13. First, the navigation camera was pulled in **under the dome**: at ORBIT_DISTANCE 1.6× the unit radius (`useConstellationNavigation` / `STAGE_CAMERA`) the near hemisphere fills the frame, the limb falls off all four edges, the zenith centers and the horizon rims — so the *steady-state framing is the looking-up point of view*, rather than a globe seen from outside. (At 2.5× the sphere read as a planet floating in the void; the diagnosis was immediate once it was seen in a screenshot. A wider FOV was tried and reverted — it lets the void back in; the narrow lens envelops.) Second, the arrival became a **stars-swell**: the firmament is mounted and present, and the structural layer fades up from nothing over the signature curve — a CSS opacity animation whose hidden start applies at first paint, so it is flash-free and works with no JS.

*What remains held* in this readiness: the overscroll-up gesture from the Foyer; the daystar's cross-page morph; and **a camera that moves on arrival** — a dolly-in beneath the dome (and, with the reflecting-pool decision in `CONSTELLATION.md`, a downward settle into the pool by day). Its one real constraint, the reason it is a deliberate next step rather than a quick swap: the structural SVG prerenders at the rest pose, so a moving-camera arrival must either start from that pose or render its start pose into the SSR — otherwise it jumps at hydration. A coordination worth verifying in the eye, not shipping blind.
...
- **No third-party rendering library beyond `ogl` (or hand-rolled).** Three.js, Babylon, Pixi, react-three-fiber, react-spring, framer-motion — all named, all refused. The bundle weight, the abstraction tax, and the *register* (these libraries carry their own visual culture) make them wrong for this surface. Even when the technical capability would help, the felt sense costs more than the help is worth. *Amended 2026-09-05, with Danny:* GSAP is admitted for one thing — the daystar's magic, the scarf of silk that swoops around the hour's face — as a lazy layer fetched after load and idle, gated by the visitor's preferences, budgeted apart from the eager path, and disposable (`PERFORMANCE_BUDGET.md` §"The Sky's Lazy Layers"). A tween engine carries no visual culture of its own; what it carries is easing, overwrite discipline, and a ticker, and hand-rolling those was the abstraction tax in the other direction. The refusal stands for everything on the first paint.
