...
1. **Persist the originals.** v4's first line says it supersedes `NORTH_STAR.md` v3 and the §28 addendum, and neither was in the house. Appendix E is a careful ledger, and a ledger is checkable only against what it ledgers. *Because:* the charter's own rule is that nothing cut is lost; the house can only verify that with the originals beside the successor. Danny has offered them; D-012 files them as `NORTH_STAR_v3.md` and `NORTH_STAR_v3.2.md` on arrival, never edited.
