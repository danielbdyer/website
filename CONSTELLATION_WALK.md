...
2. **Travel is forward.** The camera moves _along_ the thread from here to there, with depth: stars ahead grow, stars beside slide past, the deep field parallaxes. Arrival is a settle and a reveal.
3. **The sky is closed.** The sphere wraps. Every direction continues; there is no edge and no end.
4. **The visitor is always somewhere.** _Here_ is a star or the pole. Its neighborhood is named; farther stars are lights. Choosing a neighbor travels; entering _here_ opens the work.
...
**The compass is the slice's.** The eight facets are the compass the site's works hand to the sky; the sky itself reads any compass a slice carries (`CATHEDRALS.md` §"The Contract"). A vault's thirteen topic maps take thirteen bearings, evenly spaced, and the four hues are laid around them in arcs — the second of a pair, the alternates of a three or four, dotted. Every rule below that says _facet_ holds for any axis. And the relations a slice carries — the wiki links between works, or between claims — are drawn as declared threads: solid, in the page's ink, a little heavier than a figure's hairline; discovered ones dotted; the predicate is spoken by the whisper, never drawn.
...
**Gaps are honest.** Where no work points, no star sits. An empty bearing reads as _nothing yet points that way_, not as a void.
...
_Held: a frontmatter override (`sky:` with a bearing and a distance) for the rare star Danny wants to place by hand. The pull is real — "every star placed" is the authorial trace the design doc asks for — but the corpus has not yet produced a star that wants it._
...
**Derived relation is attention, not line.** Facet co-membership is a lens: it shows on hover, as the lit figure, and never as a resting mesh. The design doc's sign for threads holds: _threads were drawn, not derived_. When wikilinks arrive in the writing, they are drawn at rest between figures as the only cross-figure lines, because a hand made them.
...
**Names belong to the neighborhood, at three volumes.** Only _here_, its neighbors along the figures, and the stars its bearings lead to carry labels at rest — and they speak at three volumes: the star you stand at fullest, a neighbor one stroke off quieter, a bearing's end quieter still. Everything farther is light. Context grows exactly as far as the visitor can reach, which is what makes reaching feel like a decision. The names are laid out so none sits on another or on a star — below by habit, else above or to a side — each carrying a soft halo of the page's ground so it stays legible across a thread, and a name that must move to another side of its star glides there rather than jumping. At the overview of a crowded sky — one whose cap has grown past the default dome — no star is named; the compass's names carry the labels until the visitor enters (§"The Dial").

---

## Presence

**The sky does not show everything at once.** From where the visitor stands, at most a capped number of stars are _present_ — near in context — and the rest recede to faint lights until the walk brings them close. This is the tree that reveals itself as you move through it: a neighborhood becomes visible by being reached. At the pole, the overview, everything is present.

**Context is measured four ways and summed.** Strokes along the figures (a neighbor is nearer than a star two strokes off); facets shared; words in **concordance** — a term-frequency weight over each work's own prose, computed at build time, the library's word for a semantic nearness; and distance on the sphere. The cap always keeps _here_, its neighbors, and the stars its bearings lead to, so every offered step is visible.

**The delicious edge.** A couple of the cap's places go to the _least_ relevant stars, chosen deterministically for each place, so the sky keeps offering what is not closely related — the thing you would not have thought to look for. Relevance is not gravity; a sky that showed only the obvious would be privative.

**The concordance speaks once.** When a work's words echo the one you stand at and no facet joins them, the whisper adds a third line — _in concordance · {title}_ — and taking it travels there. This is the semantic layer made volitional rather than ambient.
...
**A destination, then motion.** Travel begins only when the visitor names where to go. The camera's surface point moves from _here_ to _there_ along the great circle in a held second or two — the _Held_ register of `CONSTELLATION_DESIGN.md` §"Motion Register", 1.1–2.4 s scaled to the distance — on a sine glide: one unbroken gesture, no change of the camera's distance. The first scaffold dollied the lens in and out across each crossing to read as depth; it read as a pulse — a neck craning forward and settling back with every hop — and was set down. Velocity is read instead from what streams past: the atmosphere's deep field streaks along the travel's angular velocity, longest at mid-crossing, still at rest. This is the trench: motion read from the walls, not the lens.
...
**Arrival acknowledges.** The destination becomes _here_: the echo widens, the label settles, the whisper changes, the new neighborhood's names fade in. The thread walked stays lit.
...
**Reduced motion** collapses travel to an instant arrival — the new _here_ simply is, with its whisper and its names.

---

## The Dial

**Nothing about the sky's size is fixed; what a person can hold at once is.** Sixteen works and two hundred and fifty-eight claims cannot share one camera. The first slice from outside the house — Danny's book vault — filled the dome so densely that every label collided, every press landed on a thread, and the whole sky read as one object. The answer is not a second geometry. It is one number, the camera's distance from the sphere's center, turned into a **dial** with two ends: far out, the whole ball fits the oculus — the _palantír_, held in the hands; at the center, the dome overhead fills the frame — the _vault_, stood inside. The sphere is the same sphere. Concave and convex are the two ends of one dial, and the event horizon is where the dial passes through the surface.

**The pole rests at the overview; a star rests at the walk.** The overview is the oculus's fit (`restDistanceFor`). The walk distance is found on the graph, not tuned per corpus: the farthest distance, up to the overview, at which about `VIEW_TARGET` stars are in frame standing at a typical star (`walkDistanceFor`, a bisection over a sample of the stars). For the site's works that distance is the overview itself, so a small sky does not change at all. For the book it is the center: the visitor stands inside the vault and the sky turns overhead as they travel. The only dolly is pole ↔ star; between stars the camera turns in place. When even the center holds more than the target, the center is the floor — the field of view past it is held, below.

**The cap grows with the count.** Each star needs a label's room at the overview (`MIN_SEPARATION`). When the default dome can no longer give it, the populated cap widens at that fixed spacing — with the square root of the count — as far as the equator (`capFor`). A sky of sixteen keeps the dome it has; a sky of hundreds takes more of the sphere; nothing is resized by hand.

**What is drawn follows the dial.** A figure's stroke is present when both its ends are; a declared thread only when, besides, one of its ends is named — so the mesh of a vault's links shows near the visitor and not as a resting web. A thread with an end behind the camera parks, because from inside the sphere it has no honest line. Only a present thread carries a hit twin, so a press between distant stars is a press on the sky, not on a line. The cursor tells the truth: default on the sky, pointer on a star, grabbing only while the hand holds. At the overview of a crowded sky no star is named — the compass's names are the labels until the visitor enters — and a star with no page of its own reads its summary in the whisper, so a claim can be read where it stands.
...
> _small weather · the Garden_
> _relation · body · becoming · language_

The first line is where you are. The second is what leads away — each facet a bearing, each a real control. A bearing with nowhere to go yet reads dim and offers nothing. At the pole the first line is _the polestar_ and the second is all eight bearings. The whisper fades through travel and returns with the new _here_. It is the only persistent text on the surface besides the return link, and it is italic, quiet, and never a sentence.

**The third line.** When a work's words are in concordance with the one you stand at and no facet joins them, the whisper adds _in concordance · {title}_, and taking it travels there (§"Presence").

---

## The Walk's Memory

Within a session the sky remembers the walk: stars the visitor has stood at stay a shade brighter, threads walked stay a shade more present, and the neighborhoods revealed stay revealed. The unexplored stays dark. This is what makes the tree feel as though it is unfolding rather than being redrawn. The memory is the session's — it dissolves between visits, per `CONSTELLATION_DESIGN.md` §"Living-Document Behavior". _Here_ persists within the session so a return from a work lands where the visitor stood.
...
- **States are representable.** Today a work is published or draft. The horizon needs a third: _proposed_ — authored by an agent, not yet blessed. Proposed stars and threads render as ghosts (dimmed, dashed, unlabeled until attended).
...
_Held until the writing and the agent both have something to propose._
...
- Where the time slider lands in the walk (it dims what did not yet exist; the walk's memory and the slider share the register of _what was_).
...
- Labels inside the vault: standing at the center the halos grow with nearness and the names' offsets do not, so _here_'s name sits in its glow. Trigger: Danny's eye inside the book sky.
- The field past the center: when the center still holds more than `VIEW_TARGET` (the book: about 48 against 32), the dial's next stop is a narrower field of view. Trigger: a slice whose center view is half again over the target.

---

## Enforced in Code

_Shipped 2026-09-01:_ the compass placement, the spread, and the facet figures live in `src/shared/content/constellation.ts`; the neighborhood, bearings (with the thread each travels along), names within a stroke, and the arrow-key choice in `src/shared/content/skyWalk.ts`; destination travel in `src/shared/hooks/useSkyTravel.ts`, which replaced the physics navigation hook (the wells, the flick, the coast, and the demonstration drift are gone); the walk's state in `src/shared/hooks/useSkyWalk.ts` and `src/shared/state/hereStorage.ts`; the input grammar in `src/shared/organisms/Constellation/useSkyInteractions.ts` (a pointer press never triggers focus-travel; only keyboard focus does); the whisper in `src/shared/molecules/SkyWhisper/`; threads as paths in `src/shared/atoms/Thread/Thread.tsx` with the projector moving the hit twin.

_Shipped 2026-09-02, the second pass with Danny:_ the dolly is gone and travel is a sine glide with the deep field's streak (`uTravel` in `src/shared/webgl/atmosphereShaders.ts`, carried by `src/shared/state/skyCamera.ts`); the rest distance adapts to the frame (`restDistanceFor`, `skyWalk.ts`); the scrub along a track lives in `useSkyTravel.ts`; the compass is drawn — meridians and sector atmospheres in the dome shader, names at the rim in `src/shared/atoms/Compass/` projected by `skyProjector.projectCompass`, dotted second figures in `Thread.tsx`; the horizon rim in the dome shader; label layout in `src/shared/dom/labelLayout.ts` applied by `skyProjector.placeLabels`; the concordance in `src/shared/content/concordance.ts` and presence in `src/shared/content/presence.ts`, with absent stars receding in both the SVG (`data-present`) and the atmosphere (`aPresence`). Not yet built: the proposed state and its author gate, the placement override, embeddings behind the concordance.

_Shipped 2026-09-02, the third pass — the oculus:_ the rest distance fits the whole sphere to the frame's shorter side (`restDistanceFor`, `limbNdcAt`, `skyWalk.ts`), the page beyond the limb and the luminous limb in the dome shader, the daystar seated on the page as the plate's corner emblem (`daystarViewboxPoint`, `skyProjector.projectDaystar`, read back by the atmosphere), the chart held still with the heavens' turn moved into the atmosphere's spin (`useWebGLFirmament.ts`), the gaze as a lean of the surface point (`useSkyTravel.ts`), labels at three volumes moved by translate (`labelLayout.ts`, `data-named` ranks), the names at the rim as bearings (`useSkyInteractions.ts`), gold returned to attention in the grid and the whisper's bearings in their hues, and the presence cap set above the corpus.

_Shipped 2026-09-03 (PR #57):_ the travel hook became a thin shell around a pure core — `src/shared/sky/motion.ts` (the camera's state machine), `src/shared/sky/hand.ts` (the drag), `src/shared/sky/walkState.ts` (the walk as a reducer), `src/shared/geometry/spring.ts` — with tests for each transition; and the drag stopped preferring the neighbor: the track no longer caps the hand at its own star, the reticle decides at release, and the graph's head start shrank. `CONSTELLATION_ARCHITECTURE.md` names the whole shape.

_Shipped 2026-09-04, the fourth pass — the dial:_ `src/shared/sky/dial.ts` (`VIEW_TARGET`, `starsInView`, `meanInView`, `walkDistanceFor`) finds the walk distance on the graph; the shell keeps two rests — the overview for the pole, the walk for a star — fitted to the frame and the graph (`fitDial`, `restFor` in `useSkyTravel.ts`); the camera aims at the surface point so the center is a place it can stand (`orbitalCamera`, `cameraAt` in `motion.ts`); the cap grows with the count (`capFor`, `constellation.ts`); a thread with an end behind the camera parks (`skyProjector.ts`); a declared thread is present only near a name (`threadWalkOf`, `Stage.tsx`); only a present thread has a hit twin (`Thread.tsx`); the cursor is default on the sky and pointer on a star (`tokens.css`); the whisper reads a page-less star's summary (`walk.ts`, `SkyWhisper.tsx`); a crowded sky names no star at the pole (`namesAt`, `skyWalk.ts`).

_Shipped 2026-09-04, the fifth pass — the frame budget:_ the shell paints only a changed motion and moves only the present stars and threads while the sky is in motion (`paintIfChanged`, `useSkyTravel.ts`; `only` in `skyProjector.ts`); a settled sky is exactly still (`settle`, `motion.ts`); a hover is written to the page and the atmosphere, never rendered (`dom/skyAttention.ts`, `state/skyHover.ts`, `useSkyAttention.ts`); the atmosphere's thread pass draws every resting hairline and the SVG paints only the lit ones (`atmosphereShaders.THREAD_VERTEX`, `tokens.css`); the hidden SVG firmament, the receded stars' bodies, and the halos the shader already paints leave the raster; the cursor parallax is gone. `CONSTELLATION_ARCHITECTURE.md` §"What Shipped (2026-09-04)" carries the measurements.

_If this document and the lived implementation disagree, the lived implementation is the present moment and this file is what it is reaching for. Catch the document up._
