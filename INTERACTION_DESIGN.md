...
| **Ascend / Descend** | The Foyer opens its ceiling; the sky closes it again | Ascending, the room crossfades (320 ms) while the nav's glyph rises and grows into the daystar's face in the sky's margin over 900 ms — the same being, in its place: by day the crown's rays carried into the sky's crown on one clock; by night the back of the moon's head turning half round to show the face. Descending, the return continues the pull: the room slides up beneath from where its ground had leaned in (640 ms) while the sky drifts up and out; the daystar stays where it is through the reveal, then travels down into the corner over 760 ms and is the glyph again | The `daystar` view-transition name, paired between the nav toggle's glyph (every route but `/sky`) and the sky's daystar button. The look-up (link or pull) keeps the default root transition. The return (link, pull, ArrowDown, Escape) sets `html.descending` for its duration, and the descent's choreography is CSS on that class. `CONSTELLATION.md` §"The Sun and the Moon". |
