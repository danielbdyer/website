...
| **Ascend / Descend** | The Foyer opens its ceiling; the sky closes it again | Ascending is a lean-back into the heavens (900 ms): the room drops away beneath and tilts, its top toward the eye, while the sky comes down from above, larger and tilted the other way, to meet the gaze; the nav's glyph rises and grows into the daystar's face in the sky's margin over the same arc — the same being, in its place; by night the far side of the moon turning half round to show the face. Descending, the return continues the pull: the room slides up beneath from where its ground had leaned in (640 ms) while the sky drifts up and out; the daystar stays where it is through the reveal, then travels down into the corner over 760 ms and is the glyph again | The `daystar` view-transition name, paired between the nav toggle's glyph (every route but `/sky`) and the sky's daystar button. The look-up (link or pull) sets `html.ascending` and the return (link, pull, ArrowDown, Escape) sets `html.descending`, each for its duration; both are choreographed in CSS on those classes. `CONSTELLATION.md` §"The Sun and the Moon". |
