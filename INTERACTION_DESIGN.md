...
| **Ascend / Descend** | The Foyer opens its ceiling; the sky closes it again | Ascending is a lean-back into the heavens (900 ms): the pull tilts the room away beneath the gaze over the sky's backdrop, and the commit continues it — the room's own image falls away and fades while the backdrop crossfades into the sky it already was; the nav's glyph rises and grows into the daystar's face in the sky's margin over the same arc — the same being, in its place; by night the far side of the moon turning half round to show the face. Descending, the sky drifts up and out and the room rises back into place beneath the gaze | The `daystar` view-transition name, paired between the nav toggle's glyph (every route but `/sky`) and the sky's daystar button. The look-up (link or pull) sets `html.ascending` and the return (link, pull, ArrowDown, Escape) sets `html.descending`, each for its duration; both are choreographed in CSS on those classes. `CONSTELLATION.md` §"The Sun and the Moon". |
