...
| **Ascend / Descend** | The Foyer opens its ceiling; the sky closes it again | Ascending is the eye turning back in a space that stays: the room, a surface continuing past the frame, falls away beneath the gaze as the pull gathers and, on commit, keeps falling over 900 ms while the heavens — a dome behind it, the sky's own canvas — come down into view by the same pitch, until the room is below the frame and the heavens stand at the sky's rest; only then does the route change. The nav's glyph falls with the page, fading at its own size, unnoticed; the character itself is already in the sky as soon as the page has uncovered its place — a fixed seat holding the daystar, face and painted body, at exactly where and how large the sky seats it, by night turning in from the edge — and the transition crosses face to face, nothing flown or grown: the same being, in its place. Descending is the same arc in reverse on the same canvas: the sky drifts up and out, the daystar holds its place while the room comes back beneath the gaze, then flies down into the corner and resolves into the glyph exactly where it rests | The `daystar` view-transition name, paired between the nav toggle's glyph (every route but `/sky`) and the sky's daystar button. The look-up (link or pull) sets `html.ascending` and the return (link, pull, ArrowDown, Escape) sets `html.descending`, each for its duration; the room's turn is CSS on `html.pulling` driven by `--reveal`, the lift and the settle are carried frame by frame in `dom/lookUp.ts`, the heavens' pitch rides the same frames, and the glyph's seat (`dom/daystarSeat.ts`) holds the character in the sky with the reveal and the icon at the glyph's rest for the face to land on. `CONSTELLATION.md` §"The Sun and the Moon". |
